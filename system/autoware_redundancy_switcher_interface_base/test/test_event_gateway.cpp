...
// ---------------------------------------------------------------------------
// MockProcessor — IProcessor のテスト用実装
// 与えたエフェクトをそのまま返し、受け取ったイベントを記録する。
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SpyAdapter — IAdapterPlugin のテスト用実装
// execute() で受け取ったエフェクトを記録する。ROS 依存なし。
// ---------------------------------------------------------------------------
...
TEST_F(EventGatewayTest, Submit_DispatchesEffectsToAdapter)
{
  ...
  EXPECT_EQ(spy_->captured().size(), 2u);
...
TEST_F(EventGatewayTest, Snapshot_ReturnsProcessorState)
{
